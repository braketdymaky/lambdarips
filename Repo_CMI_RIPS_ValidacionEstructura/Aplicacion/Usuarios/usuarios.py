...
class ValidacionUsuarios:
    nombre_archivo = 'US'
    us_model = ModeloUsuarios.obtener_instancia()
    us_model.asignar_valores([])
    metodos_de_validacion_a_ignorar = []
    lista_errores = []

    def __init__(self):
        self.us_model = ModeloUsuarios.obtener_instancia()
        self.us_model.asignar_valores(
    ...
    def validar_columnas(self,us_fila,linea_fila):
        """
        ...
        """
        try:
            if not Generico().validar_cantidad_columnas(us_fila, self.us_model.obtener_cantidad_columnas()):
                self.__detectar_error(TE.US_CANTIDAD_CAMPOS.obtener_descripcion, linea=linea_fila)
                return False
            else:
                return True
    ...
    def validar_cantidad_columnas(self, us_fila, linea_fila):
        """
        ...
        :return:
        """
        error_validar_columnas = self.validar_columnas(us_fila,linea_fila)
        if  error_validar_columnas != False:
            self.validar_longitud(us_fila, linea_fila)
            self.validar_numerico(us_fila, linea_fila)
            self.verificar_campos_no_nulos(us_fila, linea_fila)

    def validar_longitud(self, us_fila, linea_fila):
        """
        ...
        """
        try:
            for campo in self.us_model.obtener_lista_longitudes():
                valor_campo = us_fila[campo[zip.posicion_campo]]
                posicion_campo = campo[zip.posicion_campo]
                if valor_campo != '':
                    if not Generico().validar_longitud_campo(valor_campo, campo[zip.longitud_campo]):
                        self.__detectar_error(TE.US_LONGITUD_INVALIDA.obtener_descripcion, str(posicion_campo),
    ...
    def validar_numerico(self, us_fila, linea_fila):
        """
        ...
        :return:
            """
        try:
            for campo in self.us_model.obtener_campos_numericos():
                valor_campo = us_fila[campo]
                if valor_campo != '':
                    if not Generico().validar_es_numerico(us_fila[campo]):
                        self.__detectar_error(TE.US_NUMERICO.obtener_descripcion,
    ...
    def verificar_campos_no_nulos(self, us_fila, linea_fila):
        """
        ...
        """
        try:
            for index, ac_campo in enumerate(us_fila):
                if index not in self.us_model.obtener_campos_opcionales():
                    if ac_campo == '':
                        self.__detectar_error(TE.US_CAMPO_NULO.obtener_descripcion, index, linea_fila)
        except AssertionError:
            self.__detectar_error(TE.US_CAMPO_NULO.obtener_descripcion, index, linea_fila)
        except ValueError:
            self.__detectar_error(TE.US_CAMPO_NULO.obtener_descripcion, index, linea_fila)
