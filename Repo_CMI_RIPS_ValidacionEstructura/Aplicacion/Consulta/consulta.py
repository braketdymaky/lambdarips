...
class ValidacionConsulta:
    nombre_archivo = 'AC'
    ac_model = ModeloConsulta.obtener_instancia()
    ac_model.asignar_valores([])
    metodos_de_validacion_a_ignorar = []
    lista_errores = []

    def __init__(self):
        self.ac_model = ModeloConsulta.obtener_instancia()
        self.ac_model.asignar_valores(
    ...
    def validar_columnas(self,ac_fila,linea_fila):
        """
        ...
        """
        try:
            if not Generico().validar_cantidad_columnas(ac_fila, self.ac_model.obtener_cantidad_columnas()):
                self.__detectar_error(TE.AC_CANTIDAD_CAMPOS.obtener_descripcion, linea=linea_fila)
                return False
            else:
                return True
            ...
            self.__detectar_error(TE.AC_CANTIDAD_CAMPOS.obtener_descripcion, linea=linea_fila)
            return False
    def validar_cantidad_columnas(self, ac_fila, linea_fila):
        """
        ...
        :return:
        """
        error_validar_columnas = self.validar_columnas(ac_fila,linea_fila)
        if error_validar_columnas == True:
            self.validar_longitud(ac_fila, linea_fila)
            self.validar_numerico(ac_fila, linea_fila)
            self.validar_formato_fecha(ac_fila, linea_fila)
            self.verificar_campos_no_nulos( ac_fila, linea_fila)
            

    def validar_longitud(self, ac_fila, linea_fila):
        """
        ...
        """
        try:
            for campo in self.ac_model.obtener_lista_longitudes():
                valor_campo = ac_fila[campo[zip.posicion_campo]]
                posicion_campo = campo[zip.posicion_campo]
                if valor_campo != '':
                    if not Generico().validar_longitud_campo(valor_campo, campo[zip.longitud_campo]):
                        self.__detectar_error(TE.AC_LONGITUD_INVALIDA.obtener_descripcion, str(posicion_campo),
    ...
    def validar_formato_fecha(self, ac_fila, linea_fila):
        """
        ...
        """
        try:
            for campo in self.ac_model.obtener_campos_fecha():
                if campo != '':
                    if not Generico().validar_formato_fecha(ac_fila[campo]):
                        self.__detectar_error(TE.AC_FORMATO_FECHA.obtener_descripcion, str(campo), str(linea_fila))
        except AssertionError:
            self.__detectar_error(TE.AC_FORMATO_FECHA.obtener_descripcion, str(campo), str(linea_fila))
        except ValueError:
            self.__detectar_error(TE.AC_FORMATO_FECHA.obtener_descripcion, str(campo), str(linea_fila))
    ...
    def validar_numerico(self, ac_fila, linea_fila):
        """
        ...
        """
        try:
            for campo in self.ac_model.obtener_campos_numericos():
                valor_campo = ac_fila[campo]
                if valor_campo != '':
                    if not Generico().validar_es_numerico(ac_fila[campo]):
                        self.__detectar_error(TE.AC_NUMERICO.obtener_descripcion,
                                               posicion_columna=str(campo), linea=str(linea_fila))
        except AssertionError:
            self.__detectar_error(TE.AC_NUMERICO.obtener_descripcion,
                                  posicion_columna=str(campo), linea=str(linea_fila))
        except ValueError:
    ...
    def verificar_campos_no_nulos(self, ac_fila, linea_fila):
        """
        ...
        """
        try:
            for index, ac_campo in enumerate(ac_fila):
                if index not in self.ac_model.obtener_campos_opcionales():
                    if ac_campo == '':
